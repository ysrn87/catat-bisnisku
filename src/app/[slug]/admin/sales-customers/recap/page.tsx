...
async function getRecapData(storeId: string, params: {
  ...
  if (paymentStatus !== 'all') saleWhere.paymentStatus = paymentStatus;
  ...
  return {
    ...
    totalTransactions: new Set(items.map((i) => i.saleId)).size,
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardContent className="pt-4">
          <SearchFilterBar
            ...
            filters={[
              {
                key: 'period', label: 'Periode', defaultValue: 'month',
                options: [
              ...
              {
                key: 'productId', label: 'Produk', defaultValue: 'all',
                options: [
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        <Card>
          <CardContent className="pt-5 pl-3 pr-3 flex h-full items-center">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-violet-100 rounded-lg"><Receipt className="w-4 h-4 text-violet-600" /></div>
              <div><p className="text-xs text-gray-500">Jumlah Transaksi</p><p className="font-bold text-sm">{totalTransactions.toLocaleString('id-ID')}</p></div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-5 pl-3 pr-3 flex h-full items-center">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-blue-100 rounded-lg"><ShoppingBag className="w-4 h-4 text-blue-600" /></div>
              <div><p className="text-xs text-gray-500">Total Item Terjual</p><p className="font-bold text-sm">{totalQty.toLocaleString('id-ID')}</p></div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-5 pl-3 pr-3 flex h-full items-center">
            <div className="flex items-center gap-2">
              ...
              <div><p className="text-xs text-gray-500">Total Pendapatan</p><p className="font-bold text-xs">{formatRupiah(totalRevenue)}</p></div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-5 pl-3 pr-3">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-orange-100 rounded-lg"><BarChart2 className="w-4 h-4 text-orange-600" /></div>
              <div className="min-w-0">
                <p className="text-xs text-gray-500">Produk Terlaris</p>
                {topProduct ? (
                  <><p className="font-semibold text-[10px] line-clamp-2">{topProduct.variantName}</p><p className="text-[10px] text-amber-500 truncate">{topProduct.productName} - {topProduct.sku}</p></>
