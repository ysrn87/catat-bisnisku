...
export default async function CashierLayout({ children, params }: { children: React.ReactNode; params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const session  = await auth();
  if (!session) redirect(`/login?callbackUrl=/${slug}/cashier`);

  // UPDATED: query ke StoreStaff
  const staffRecord = await db.storeStaff.findFirst({
    where: { userId: session.user.id, store: { slug }, role: { in: ['OWNER', 'ADMINISTRATOR', 'MANAGER', 'CASHIER'] } },
