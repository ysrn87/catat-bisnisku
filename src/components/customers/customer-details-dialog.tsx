...
import {
  ShoppingCart, Package, TrendingUp, Star, Sparkles,
  Phone, MapPin, Mail, User, UserCog, Pencil,
} from 'lucide-react';
...
import { deleteWalkInCustomerAction } from '@/actions/customers';
import type { MemberCustomer, NonMemberCustomer, Customer } from './customers-table';

export type { MemberCustomer, NonMemberCustomer, Customer };
...
function StatCard({ icon, label, value, sub, color }: {
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl p-4 ${color}`}>
      ...
      <p className="text-xl sm:text-2xl font-bold text-white tracking-tight leading-none mb-1 break-all">{value}</p>
            ...
            {sale.items.map((item: any) => (
              <div key={item.id} className="flex justify-between items-start gap-3 px-3 sm:px-4 py-2.5">
                <div className="min-w-0 text-xs">
                  <span className="text-gray-800">{item.variant.name}</span>
                  <span className="text-gray-400 mx-1">·</span>
                  <span className="text-amber-600 ml-1 text-[10px]">×{item.quantity}</span>
                  <p className="text-gray-500">{item.variant.product.name}</p>
                </div>
        ...
        <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 px-4 sm:px-6 pt-5 pb-4 overflow-hidden shrink-0">
          ...
          <div className="relative flex items-start gap-3 pr-10">
            ...
            <div className="flex-1 min-w-0 pt-0.5">
              ...
              {/* badge: sudah punya akun tapi belum upgrade */}
              {!isMember && (customer as NonMemberCustomer).hasAccount && (
                <div className="mt-2 inline-flex items-center gap-1.5 bg-blue-400/15 border border-blue-400/25 rounded-full px-2.5 py-0.5">
                  <span className="text-blue-300 text-[10px] font-semibold">Sudah punya akun · Bisa diupgrade</span>
                </div>
              )}
            </div>
          </div>
          ...
          {showActions && (
            <div className="relative mt-3 flex items-center gap-2">
              {isMember ? (
                <>
                  <Button
                    variant="ghost" size="sm" title="Edit Member"
                    className="group" onClick={() => setEditOpen(true)}
                  >
                ...
                <>
                  ...
                  />
                  <Button
                    variant="ghost" size="sm" title="Upgrade ke Member"
                    className="group" onClick={() => setUpgradeOpen(true)}
                  >
                  ...
                  <DeleteConfirmDialog
                    ...
                    description="Yakin ingin menghapus pelanggan ini? Tidak dapat dihapus jika punya riwayat transaksi."
                    onConfirm={() => deleteWalkInCustomerAction(customer.id)}
                  />
                </>
              )}
            </div>
          ...
          <div className="relative mt-4 grid grid-cols-3 gap-1.5 sm:gap-2">
            {[
              { label: 'Transaksi',     value: customer._count.sales.toString(),  icon: <ShoppingCart className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-300" /> },
              { label: 'Total Belanja', value: formatCurrency(totalSpent),        icon: <TrendingUp   className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-300" /> },
              { label: 'Rata-rata',     value: formatCurrency(avgPurchase),       icon: <Star         className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-300" /> },
            ].map((s) => (
              <div key={s.label} className="bg-white/[0.08] backdrop-blur-sm border border-white/10 rounded-xl px-2 sm:px-3 py-2 sm:py-2.5">
                <div className="flex items-center gap-1 mb-1">
                  {s.icon}
                  <span className="text-[8px] sm:text-[10px] text-slate-300 uppercase tracking-wider leading-none">{s.label}</span>
                </div>
                <p className="text-[11px] sm:text-sm font-medium text-white break-all leading-snug">{s.value}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <Tabs defaultValue="profile" className="flex-1 flex flex-col overflow-hidden min-h-0">
          ...
          <TabsContent value="profile" className="flex-1 overflow-y-auto px-4 sm:px-6 pb-6 mt-3 min-h-0">
            {isMember ? (
              ...
              <div className="space-y-2.5">
                {[
                  { icon: <Phone  className="w-4 h-4 text-emerald-600" />, bg: 'bg-emerald-50', label: 'Telepon',         value: customer.phone || undefined },
                  { icon: <MapPin className="w-4 h-4 text-rose-500"   />, bg: 'bg-rose-50',    label: 'Alamat',          value: customer.address ?? undefined },
                  { icon: <User   className="w-4 h-4 text-amber-600"  />, bg: 'bg-amber-50',   label: 'Customer Sejak',  value: formatFullDate(customer.createdAt) },
                ].filter((r) => r.value).map((row) => (
                  <div key={row.label} className="flex items-start gap-3 p-3 sm:p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-800 break-words">{row.value}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <TabsContent value="statistics" className="flex-1 overflow-y-auto px-4 sm:px-6 pb-6 mt-3 min-h-0">
            <div className="grid grid-cols-2 gap-2.5 mb-2.5">
              <StatCard icon={<ShoppingCart className="w-4 h-4 text-white" />} label="Total Transaksi" value={customer._count.sales.toString()} color="bg-gradient-to-br from-violet-500 to-indigo-600" />
              <StatCard icon={<TrendingUp   className="w-4 h-4 text-white" />} label="Total Belanja"   value={formatCurrency(totalSpent)}        color="bg-gradient-to-br from-emerald-500 to-teal-600" />
            </div>
            <StatCard
          ...
          <TabsContent value="history" className="flex-1 overflow-y-auto pb-6 mt-3 min-h-0">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-gray-400 gap-3">
                <div className="p-4 rounded-2xl bg-gray-100"><Package className="w-5 h-5" /></div>
              ...
              <div className="px-1 sm:px-2">
                {purchaseHistory.map((sale, i) => (
                  <div key={sale.id}>
                    ...
                    {i < purchaseHistory.length - 1 && <div className="mx-3 sm:mx-4 h-px bg-gray-100" />}
                  </div>
                ))}
              </div>
            )}
